...
CodGeneration::Generation::Generation(LT::LexTable lexT, IT::IdTable idT, std::string outFile)
{
	lexTable = lexT;
	idTable = idT;
...
void CodGeneration::Generation::start()
{
	head();
	constants();
	data();
...
void CodGeneration::Generation::head()
{
	out << ".586\n";
...
#ifdef _DEBUG
	out << "includelib ../Debug/SPA_Lib.lib\n";
#endif
#ifndef _DEBUG
	out << "includelib SPA_Lib.lib\n";
#endif
	out << "ExitProcess PROTO : DWORD\n";
	out << "_convert PROTO : DWORD\n";
	out << "_strln PROTO : DWORD\n";
	out << "_out PROTO : DWORD\n";
	out << "_outbool PROTO : DWORD\n";
	out << "_outpint PROTO : DWORD\n\n";
	out << "\n.stack 4096\n";
}

void CodGeneration::Generation::constants()
{
	out << ".const\n";
	out << "\t_DIVISION_BY_ZERO_ERROR BYTE 'Деление на ноль запрещено', 0\n";
	out << "\t_OVERFLOW_ERROR BYTE 'Переполнение', 0\n";
...
void CodGeneration::Generation::data()
{
	out << ".data\n";
	for (int i = 0; i < idTable.size; i++)
	{
...
void CodGeneration::Generation::code()
{
	out << "\n.code\n";
	int indOfFunc = -1;
	int indOflex = -1;
	...
	for (int i = 0; i < lexTable.size; i++) {
		...
		case LEX_EQUAL: {
			...
			while (lexTable.table[i].lexema != LEX_SEMICOLON) {
				...
				if (lexTable.table[i].lexema == LEX_OST)
				{
					out << "\n";
					out << "\tpop\t\tebx\n";
					out << "\tpop\t\teax\n";
				...
				if (lexTable.table[i].lexema == LEX_FULL_EQUALS)
				{
					...
					out << "\tpop\t\teax\n";
					out << "\tcmp\t\teax, ebx\n";
					out << "\tjz\t\tNOTEQUAL" << lexTable.table[i].sn << "\n";
					out << "\tpush\t\t1\n";
					out << "\tjnz\t\tEQUAL" << lexTable.table[i].sn << "\n";
...
void CodGeneration::Generation::end()
{
